...
3) If multiple topics match, pick the most specific by keyword uniqueness priority: Zultys > VoIP > Firewalls > Intune > Vulnerability Review > Microsoft Certifications > Code Review > Persona Formatting > General Triage.
...
2. Logic Audit: Pass the output to @Reviewer.md (OmniCoder) to check for bugs, efficiency, security, and adherence to requirements. Reviewer must return a "Pass/Fail" with minimal-diff fix patches.
...
- citations_required: true for security/diagnostics topics

Quality Gates
- Vendor-verification required for: @Firewall.md, @Intune_Analyst.md, @Zultys.md, @Vuln_Reviewer.md.
...
- @Docuwriter.md must embed version, last_updated, and pipeline trace in the documentation header.
