...
<#
.SYNOPSIS
    Copies SharePoint calendar items into a Microsoft 365 group calendar and preserves rollback state.

.DESCRIPTION
    This script supports three modes:
      - SaveState: inventories SharePoint calendar items and writes a JSON state file.
      - Migrate: creates matching events in the target Microsoft 365 group calendar.
      - Restore: removes the previously created events using the saved state.
